...
class CutPasteLayer(torch.nn.Module):
    def __init__(self, area_ratio=[0.02,0.15], aspect_ratio=0.3, rotation=[-45,45], width=[7,40], height=[10,40]):
    ...
    def normal(self, x):
        ...
        for i in range(batch_size):
            ratio_area = random.uniform(self.area_ratio[0], self.area_ratio[1]) * height * width
            ...
            from_location_h = int(random.uniform(0, height - cut_h))
            from_location_w = int(random.uniform(0, width - cut_w))

            to_location_h = int(random.uniform(0, height - cut_h))
            to_location_w = int(random.uniform(0, width - cut_w))

            x[i, :, to_location_h:to_location_h + cut_h, to_location_w:to_location_w + cut_w] = x[i, :, from_location_h:from_location_h + cut_h, from_location_w:from_location_w + cut_w].clone()
        return x

    def scar(self, x):
        # color_jitter = TL.ColorJitterLayer(brightness=0.9, contrast=0.9, saturation=0.1, hue=0.1, p=1.0)
        #transform_ = nn.Seq    uential(
        #    color_jitter,)
        ...
        for i in range(batch_size):
            ...
            while True:
                ...
                pixel_avg = torch.mean(x[i, :, to_location_h:to_location_h + cut_h,  to_location_w:to_location_w + cut_w]).item()
                if ((pixel_avg > 0.15) and (pixel_avg < 0.8)) or (max_iter > 20):
                    if (max_iter > 20):
                        print("max_iter:)")
                    break
            ...
            if r > 0.15:
                ...
                img_mask_rotated = self.transform_(img_mask_rotated.unsqueeze(0).cpu()).squeeze(0).to(self.device)
